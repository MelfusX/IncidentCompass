...
- Non-gated local smoke improved from the Phase 2 baseline (`qwen2.5-14b-instruct`: 0/N useful completions) to Phase 3 `3/3` `publish_report` completions with `MaxReprompts: 2`. Phase 4 real-LLM smoke was not executed because no local OpenAI-compatible endpoint responded. These are opt-in smoke evidence only, not CI release gates.
