...
Use `Query.cs` instead of `Command.cs` for read-only use cases. If a type name such as `GetCurrentUserQuery` is needed to understand the behavior, the folder structure or use-case name is probably too vague.
