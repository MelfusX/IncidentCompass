...
Tool execution must go through backend policy. Risky tools require approval or must be rejected. The LLM must not receive infrastructure credentials. The Governance subsystem (`IncidentCompass.Application/Governance`) implements this policy/audit boundary; it currently has no caller, since its prior caller (the chat loop) has been removed. A future phase wires a new caller into it.
