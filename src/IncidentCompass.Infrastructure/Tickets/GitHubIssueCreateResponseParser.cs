...
public static class GitHubIssueCreateResponseParser
{
    ...
    public static bool TryValidateCanonicalResult(byte[] payload, out byte[] canonical)
    {
        canonical = [];
        try
        {
            ...
            if (root.ValueKind != JsonValueKind.Object || root.EnumerateObject().Count() != 2 ||
                ...
                !int.TryParse(number.GetString(), out var issueNumber) || issueNumber <= 0)
            {
                return false;
            }

            canonical = Encoding.UTF8.GetBytes(CanonicalJsonSerializer.Canonicalize(JsonNode.Parse(payload)));
            return payload.AsSpan().SequenceEqual(canonical);
        }
        catch (JsonException)
        {
            return false;
